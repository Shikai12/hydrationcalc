...
class GlobalGuard:
    ...
    def validate(self, raise_on_error: bool = True) -> List[str]:
        """
        校验数据完整性
        对自动创建的空站点宽容处理（author_info/persona 为空时不检查子字段）
        """
        errors = []
        ...
        for site_key, site_data in sites.items():
            ...
            # 检查 author_info（如果存在且非空，则检查子字段；空字典跳过）
            if 'author_info' in site_data and site_data['author_info']:
            ...
            # 检查 persona（如果存在且非空，则检查子字段；空字典跳过）
            if 'persona' in site_data and site_data['persona']:
