...
# -*- coding: utf-8 -*-
"""
build.py -- V4.0 Single Site Mode (NO BACKUP - for GitHub Actions/repo)
...
def sync_site_global(site_data, articles_json):
    ...
    for art in articles_json:
        ...
        normalized = {
            'slug': slug,
            'title': art.get('title', ''),
            'date': art.get('date', art.get('date_published', datetime.now().isoformat())),
            'date_iso': art.get('date_iso', ''),
            'date_published': art.get('date_published', ''),
            'date_modified': art.get('date_modified', ''),
            'category': art.get('category', ''),
            'word_count': art.get('word_count', 0),
            'author': art.get('author', site_data.get('persona', {}).get('name', '')),
        ...
        if isinstance(normalized['author'], str):
            normalized['author'] = {
                ...
                'job_title': site_data.get('persona', {}).get('job_title', '')
            }
        site_data['articles'].append(normalized)
        existing_slugs.add(slug)
        new_count += 1
        print(f"[NEW] Article added to global: {slug}")
...
def main():
    try:
        print("[BUILD] build.py V4.0 Single Site Mode (NO BACKUP)")
        ...
        print("       (backup disabled - repo mode)")
